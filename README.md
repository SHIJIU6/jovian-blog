...
- **前台布局优化**：项目、资源推荐、博客博主等数据列表统一使用固定高度内容区，列表内容在区域内滚动，避免页面被长列表撑开。
- **留言板交互优化**：留言板与上方数据列表分离展示，展开时向下展开，并保持底部区域展示，不遮挡上方内容。
- **后台新增入口**：项目模块、资源模块、博客博主模块在非编辑模式下也提供新增能力，方便直接录入新内容。
- **字段可空策略**：新增项目、资源、博主与站点背景图片等字段支持为空保存，减少后台录入时的必填限制。
- **图片上传完善**：项目图片、资源 logo、博主头像与站点配置上传能力统一，保存后使用 `/api/media/...` 访问，避免刷新后仍保留临时 `blob:` 地址。
- **空图片保护**：图片地址为空时不渲染空 `src`，避免浏览器出现空字符串图片地址警告。
- **文章留言管理**：后台文章管理操作列支持打开留言面板，查看并管理对应文章的留言列表。
- **文章归档展示**：前台与后台文章列表默认按月份维度展示，而不是按年份聚合。
...
- 项目、资源推荐、博客博主模块均支持在后台直接新增内容，不需要先进入编辑模式。
- 新增表单中的标题、描述、链接、图片、标签、评分等字段尽量允许为空，适合先占位保存再逐步完善。
- 前台项目、资源推荐、博客博主列表统一使用固定高度显示区域；内容超出时在列表区域内部滚动，保证留言板和页面布局稳定。
- 留言板作为独立区域展示在数据列表下方，展开留言时向下展开，不覆盖上方数据内容。
- 文章管理列表的操作列提供留言查看入口，可打开对应文章的留言列表并执行管理操作。
...
- `/api/comments`：前台留言读取与提交接口，支持按目标类型和目标 ID 关联文章、项目、资源、博主等内容。
- `/api/admin/comments/*`：后台留言管理接口，用于查看、审核、隐藏或删除留言。
- `/api/admin/assets/upload`：后台媒体上传接口，用于站点配置、项目、资源、博主等模块图片上传。
- `/api/media/*`：媒体访问接口，本地开发读取 `.local-content/media/`，生产环境读取 Cloudflare R2。
...
- [x] 音乐清单与 SVG 图标生成脚本。
- [x] 项目、资源推荐、博客博主模块后台新增能力。
- [x] 项目、资源推荐、博客博主前台固定高度滚动列表。
- [x] 通用留言板与后台文章留言查看管理。
- [x] D1 留言表迁移与留言接口。
- [x] 项目、资源、博主与站点配置媒体上传链路。
- [x] 新增表单字段可空保存与空图片地址保护。
- [x] 前台与后台文章列表按月份维度展示。
...
### 留言保存失败：`D1_ERROR: no such table: comments`

说明当前 D1 数据库还没有应用留言表迁移。请执行：
...
### 上传图片显示保存成功但刷新后仍是无效图片怎么办？

请检查保存后的内容是否已经从临时 `blob:` 地址替换为 `/api/media/...` 地址；同时确认本地 `.local-content/media/` 或生产环境 R2 bucket `BLOG_MEDIA` 中存在对应文件。

### 控制台提示空字符串传给图片 `src` 怎么处理？

图片字段允许为空时，组件不应渲染 `<img src="" />`。应在图片地址存在时再渲染图片，或显示明确的占位状态。
